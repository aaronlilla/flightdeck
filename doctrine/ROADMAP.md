...
| R-46 | every console surface serves three registers -- a glance sentence, a detail sentence behind a disclosure, and the raw fact record under `?verbose=1` -- written by the model through `reasonerFor` class `narrate`, refereed against the facts before it is served, cached by the facts and never by the clock, with no route ever awaiting a call | autonomy | review | #106 | checker specimens (dropped PR number, moved clock time, invented count, swapped state word, leaked run id) each named their token then the design's own copy accepted; store falsifiers (never-resolving queryFn, one call for two reads, zero on a second poll, the 301st key capped, restart makes zero calls for known keys); live proof on 2026-09-09 against the real model: 20 `reasoner.call` rows on the first pass, 0 on the second, 19/20 accepted |
| R-47 | the width stepper answers the click at once and posts once when the clicking stops | autonomy | running | this PR | six to ten is one post of 10, and the number moves before the round trip |
| R-48 | every Jira comment and PR write flightdeck makes passes the Joe readability rule, refusing on DENY, contract and specimens loaded from the machine rather than shipped in the repo | autonomy | review | this PR | refusal tests watched red then green for the Jira write client, the PR-comment writer and the worker's own `gh pr create`/`comment`; a neutral in-repo specimen set for the repo's own tests, the real contract installed to `~/.forge/readability` by dev-harness |
| R-53 | the console as a Windows service with graceful stop (SIGINT/SIGTERM/SIGBREAK clear the tick, close the server, release the queue lock, journal `console.stopped`); a logon-time login helper for the browser step a service in Session 0 cannot do | autonomy | review | this PR | `tests/forge/service/shutdown.test.ts`, `tests/forge/service/install-script.test.ts`, `tests/forge/login-helper.test.ts`, `tests/forge/server-login-helper.test.ts` red then green; the NSSM zip's sha256 verified by direct download and hash |
| R-54 | the drift judge reads the run's literal transcript tail, not just tool names, and acts: `drifting` nudges, a confirmed `off-brief` parks | autonomy | review | this PR | `tests/forge/transcript-drift.test.ts` red then green, including the off-brief specimen invisible in tool names whose unique path is asserted present in the judge's actual prompt |
| R-55 | per-run wall clock (`RunState.startedAt`, `maxWallMs` per class, parks once) and an orphan sweep that only ever touches a finished run's own process subtree | autonomy | review | this PR | `tests/forge/session-clock.test.ts`, `tests/forge/sweep.test.ts` red then green, including the deep-ancestor-chain specimen (a pid reused by a live session two levels up from the target) |
| R-56 | an async Codex advisor (`ask`/`status`/`result`) the Conductor agent can call as `ask_codex`, never blocking a tick | autonomy | review | this PR | `tests/forge/council/codexAdvisor.test.ts` red then green (`ask` returns before `result` would ever resolve); one live `ping`-class call, result `pong`, exit 0 |
| R-59 | flightdeck gates its own Jira and PR writes on the Joe rule again, contract and specimens loaded from the machine | autonomy | review | this PR | agnostic check exit 0; neutral specimen suite pinned at 8; installer test red then green; unconfigured journals `readability.unconfigured` and a status line is unit-tested (not yet a console row -- see `console/readability-status.ts`) |
