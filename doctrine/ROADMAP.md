...
| R-44 | account limits read live: every window the provider reports (session, weekly, weekly per model) with its percent and reset time, on the Settings row and in account selection | autonomy | review | this PR | `accounts-probe.ts` reads Claude's `oauth/usage` limits array and ChatGPT's `wham/usage`; specimen bodies in `tests/forge/accounts-probe.test.ts`; the 1e pair shows three rows with bars and reset times |
