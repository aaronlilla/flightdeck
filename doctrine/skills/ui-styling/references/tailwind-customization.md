...
The v4 font-size namespace is `--text-*` (generates `text-xs`,
`text-jumbo`, ...); `--font-*` is the font-family namespace, and a
`--font-size-*` variable generates nothing:
...
@utility tab-* {
  tab-size: --value(integer); /* functional utilities use --value(), not var(); matches tab-4, tab-8, ... */
}
...
## Config-File Sections Below: Tailwind v3 (legacy)

Everything below that reaches for `tailwind.config.js/ts` is the v3
mechanism. On Tailwind v4: plugins load in CSS
(`@plugin "@tailwindcss/typography";`), the class-based dark variant is
`@custom-variant dark (&:is(.dark *));`, extra content paths use
`@source`, and a config file is opt-in via `@config`. The shadcn
`hsl(var(--...))` colors block in the example config is the v3 shadcn
convention only — on v4 there is no colors block at all; see
`shadcn-theming.md`.
