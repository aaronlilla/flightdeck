# E2E console probe (run 2: throwaway, pause/resume target)
...
5. Run the Bash command `sleep 15 && echo tick5`. Then write: `tick 5`.
6. Run the Bash command `sleep 15 && echo tick6`. Then write: `tick 6`.
7. Run the Bash command `sleep 15 && echo tick7`. Then write: `tick 7`.
8. Run the Bash command `sleep 15 && echo tick8`. Then write: `tick 8`.
9. Call the tool `mcp__forge__forge_done` with the evidence text `probe complete: 8 ticks`.
   Then stop.
