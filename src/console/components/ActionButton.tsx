  ...
  if (result.kind === 'confirm') {
    return (
      <span
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 'var(--fs-ui)', padding: '4px 8px', border: '1px solid var(--block)', borderRadius: 4 }}
        onClick={(e) => { if (stopPropagation) e.stopPropagation(); }}
      >
        <span style={{ color: 'var(--ink2)' }}>Confirm, irreversible: {result.blast}</span>
        <span className="btnR" style={{ padding: '4px 8px', fontSize: 'var(--fs-ui)' }} data-testid={`action-confirm-yes-${suffix}`} {...actionable(guard(onConfirm))}>Confirm</span>
        <span className="btnS" style={{ padding: '4px 8px', fontSize: 'var(--fs-ui)' }} data-testid={`action-confirm-no-${suffix}`} {...actionable(guard(onDismiss))}>Not now</span>
      </span>
    );
  }
  return (
    <span
      ...
      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 'var(--fs-ui)', color: result.ok ? 'var(--run)' : 'var(--block)' }}
      onClick={(e) => { if (stopPropagation) e.stopPropagation(); }}
    >
