...
export function Toast({ toast }: ToastProps): JSX.Element | null {
  ...
  return (
    ...
    >
      <span className="m" style={{ fontSize: 22, color: toast.color ?? 'var(--run)' }}>{toast.glyph}</span>
      <div style={{ flex: 1 }}>
        <div className="m" style={{ fontSize: '12.5px', fontWeight: 700 }}>{toast.title}</div>
        <div className="m" style={{ fontSize: '10.5px', color: 'var(--ink2)' }}>{toast.sub}</div>
      </div>
      <span className="m" style={{ fontSize: 26, fontWeight: 700, color: toast.color ?? 'var(--run)' }}>{toast.big}</span>
    </div>
  );
}
