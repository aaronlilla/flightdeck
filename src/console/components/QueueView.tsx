...
function LaterRow({ item }: { item: QueueItem }): JSX.Element {
  ...
  const action = item.state === 'review'
    ...
    : item.state === 'parked' || item.state === 'failed'
      ...
      : null;
  return (
    <div className="lane" style={{ display: 'grid', gridTemplateColumns: '44px 1fr 1fr 190px', gap: 18, alignItems: 'baseline', padding: '14px 16px', borderBottom: '1px solid var(--line)' }}>
...
export function QueueView({ items, paused, pauseReason, maxInFlight, working = 0 }: QueueViewProps): JSX.Element {
  ...
  return (
    <main data-testid="queue-view" className="scroll" style={{ flex: 1, minWidth: 0, overflow: 'auto', padding: '26px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {next.map((item, index) => (
          <div key={item.id} className="lane" style={{ display: 'grid', gridTemplateColumns: '44px 1fr 1fr 190px', gap: 18, alignItems: 'baseline', padding: '14px 16px', borderBottom: '1px solid var(--line)' }}>
