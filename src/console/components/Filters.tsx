...
export function Filters({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px 0', flexWrap: 'wrap' }}>
      ...
      <span style={{ flex: 1 }} />
      <ActionButton spec={ACTIONS.postRetireFinished} args={[]} actionRef="filters" className="btnS" style={{ padding: '6px 10px', fontSize: 'var(--fs-ui)' }} busy="Cleaning up…">Clean up</ActionButton>
      <ActionButton spec={ACTIONS.postMergeReady} args={[]} actionRef="filters" className="btnP" style={{ padding: '6px 10px', fontSize: 'var(--fs-ui)' }} busy="Merging…">Merge ready</ActionButton>
      <span className="m" style={{ fontSize: 'var(--fs-ui)', color: 'var(--ink2)', whiteSpace: 'nowrap' }}>
        sort:
        {SORTS.map((s) => (
          <a
            key={s.key}
