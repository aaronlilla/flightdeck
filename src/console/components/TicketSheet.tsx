    ...
    <div style={{ marginBottom: 16 }}>
      {story.entries.length > 0 ? (
        <>
          ...
          <div className="m" style={{ fontSize: 'var(--fs-meta)', lineHeight: 2, color: 'var(--ink2)', overflowWrap: 'anywhere' }}>
      ...
      {story.brief ? (
        <div style={{ marginTop: 10 }}>
          ...
          {briefOpen ? <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink2)', marginTop: 6 }}>{story.brief.excerpt}</div> : null}
        </div>
      ) : null}
    </div>
  );
  ...
  if (loadFailed) {
    return (
      <div data-testid="ticket-sheet-summary" style={{ padding: '18px 22px', borderBottom: '1px solid var(--line)' }}>
        ...
        <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--block)' }}>could not load the summary.</div>
      </div>
    );
  }
  if (!summary) {
    return (
      <div data-testid="ticket-sheet-summary" style={{ padding: '18px 22px', borderBottom: '1px solid var(--line)' }}>
        <div className="lbl" style={{ color: 'var(--ink2)', marginBottom: 10 }}>Summary</div>
        <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink3)' }}>Checking the PR, its checks and the audit…</div>
      </div>
    );
  }
    ...
    <div data-testid="ticket-sheet-summary" style={{ padding: '18px 22px', borderBottom: '1px solid var(--line)', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        ...
        {summary.what.length > 0 ? (
          <ul className="m" style={{ margin: 0, paddingLeft: 18, fontSize: 'var(--fs-meta)', color: 'var(--ink)', lineHeight: 1.6 }}>
            {summary.what.map((line, i) => <li key={i}><Linkify text={line} repo={repo} /></li>)}
          </ul>
        ) : (
          <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink3)' }}>Nothing on record yet.</div>
        )}
      </div>
      <div>
        <div className="lbl" style={{ color: 'var(--ink2)', marginBottom: 10 }}>Where it is</div>
        <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink2)', marginBottom: 6 }}><Linkify text={summary.status} repo={repo} /></div>
        <div
          ref={auditRef} data-testid="ticket-sheet-audit" className="m"
          style={{
            fontSize: 'var(--fs-meta)', color: 'var(--ink2)', marginBottom: 6,
        ...
        <div data-testid="ticket-sheet-readiness" className="m" style={{ fontSize: 'var(--fs-meta)' }}>
          {readiness?.ok ? (
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <span className="m" data-testid="ticket-sheet-next" style={{ fontSize: 'var(--fs-meta)', fontWeight: 700, color: 'var(--ink)' }}><Linkify text={summary.next} repo={repo} /></span>
          <span style={{ flex: 1 }} />
          <span
            className="btnS"
            style={{ padding: '6px 10px', fontSize: 'var(--fs-meta)', opacity: recheckRunning ? 0.55 : 1, display: 'inline-flex', alignItems: 'center', gap: 6 }}
          ...
          <span
            ...
            style={{ padding: '6px 10px', fontSize: 'var(--fs-meta)', opacity: reauditRunning ? 0.7 : 1, cursor: reauditRunning ? 'default' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}
      ...
      <div className="scroll" style={{ flex: '0 1 auto', minHeight: 0, overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <a className="m" style={{ fontSize: 'var(--fs-meta)' }} {...actionable(() => onOpenSandbox(lane.id))}>sandbox</a>
            {verbose ? (
              <span
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end' }}>
            <LaneCta lane={lane} cmd={cta.cmd} label={cta.label} cls={cta.cls} style={{ padding: '7px 11px', fontSize: 'var(--fs-meta)' }} onCommand={onCommand} />
            {canPause ? <LaneCta lane={lane} cmd="pause" label="Pause" cls="btnS" style={{ padding: '7px 11px', fontSize: 'var(--fs-meta)' }} onCommand={onCommand} /> : null}
            {canKill ? <LaneCta lane={lane} cmd="kill" label="Kill" cls="btnR" style={{ padding: '7px 11px', fontSize: 'var(--fs-meta)' }} onCommand={onCommand} /> : null}
          </div>
        </div>
      </div>
              ...
              <div key={name} style={{ display: 'contents' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, width: 112 }}>
                  <div style={{
                    ...
                    font: '700 var(--fs-ui) "IBM Plex Mono",monospace', color: style.glyphColor, animation: style.anim,
                  }}
                  >
                    {style.glyph}
                  </div>
      ...
      <div data-testid="ticket-sheet-body" style={{ display: 'flex', flex: '1 0 280px', overflow: 'hidden' }}>
          ...
          {lane.pr ? (
            <>
              ...
              <div className="plate" style={{ padding: '10px 12px' }}>
                <a
                  className="m" style={{ fontSize: 'var(--fs-meta)', fontWeight: 700 }}
        ...
        <div style={{ flex: '2 1 380px', minHeight: 0, padding: '16px 22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ flex: 'none', background: 'var(--well)', boxShadow: 'inset 0 2px 5px rgba(0,0,0,.6)', borderRadius: 3, padding: '8px 8px 8px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            />
            <span
              className="btnP" style={{ padding: '5px 10px', fontSize: 'var(--fs-meta)', opacity: sending ? 0.55 : 1 }}
            ...
            <span
              className="btnS" style={{ padding: '5px 10px', fontSize: 'var(--fs-meta)', opacity: amend.pending ? 0.55 : 1 }}
