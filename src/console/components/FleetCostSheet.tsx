...
export function FleetCostSheet({ lanes, spentTodayUsd, onClose, onOpenLane }: FleetCostSheetProps): JSX.Element {
  const sorted = [...lanes].sort((a, b) => b.costUsd - a.costUsd);
  return (
    <div className="plate" data-testid="fleet-cost-sheet" style={{ width: 640, maxWidth: 'calc(100vw - 40px)' }}>
      ...
      <div style={{ padding: '18px 22px 8px', display: 'flex', alignItems: 'baseline', gap: 14 }}>
        ...
        <span className="w0" style={{ fontSize: 22 }}>${spentTodayUsd.toFixed(2)}</span>
      </div>
      <div className="scroll" style={{ maxHeight: 360, overflow: 'auto', padding: '4px 0 12px' }}>
        {sorted.map((lane) => (
          ...
          >
            ...
            <span className={costClass(lane)} style={{ fontSize: 12, padding: '2px 6px' }}>${lane.costUsd.toFixed(2)}</span>
