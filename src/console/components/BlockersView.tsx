...
function StepButtons({ blocker, enabled }: { blocker: Blocker; enabled: boolean }): JSX.Element {
  ...
  if (result?.kind === 'resolved') {
    ...
    return (
      <span className="m" style={{ color: 'var(--run)', fontSize: 'var(--fs-meta)' }}>
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
      {blocker.youCanResolve ? (
        <ActionButton
          ...
          style={{ padding: '7px 11px', fontSize: 'var(--fs-meta)' }} busy="Checking…" outcome="none" onOutcome={onOutcome}
        >
          Resolved, check it
        </ActionButton>
      ) : null}
      ...
      {result?.kind === 'not-yet' ? (
        <span className="m" data-testid={`blocker-not-yet-${blocker.id}`} style={{ color: 'var(--block)', fontSize: 'var(--fs-meta)' }}>Not yet: {(result.detail ?? 'not confirmed').replace(/^not yet: /, '')}</span>
      ) : null}
    </div>
  );
}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
        <div className="m" style={{ fontSize: 'var(--fs-meta)', fontWeight: 700, display: 'flex', gap: 8, alignItems: 'baseline' }}>
      ...
      <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink2)' }}>
        {linkify(blocker.detail, blocker.links, jiraSite, `${blocker.id}-detail`)}
      </div>
      <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink2)' }}>To resolve: {blocker.howToResolve}</div>
      <div className="m" style={{ fontSize: 'var(--fs-meta)', color: 'var(--ink3)' }}>Then: {blocker.thenWhat}</div>
