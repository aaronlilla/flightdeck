...
export type QueueLoopOutcome = 'ok' | 'failed' | 'paused';

export interface QueueLoopStatus {
  /** Plain English, no identifier in it: safe to put straight on a screen. */
  sentence: string;
  /** Epoch millis of the last pass that finished, either way. Null before the first. */
  lastCompletedAt: number | null;
  lastOutcome: QueueLoopOutcome | null;
  /** The message of the last failure, when the last pass failed. */
...
export class QueueTickRunner<T> {
  ...
  private sentence(sinceMs: number, overdue: boolean): string {
    ...
    if (overdue) {
      ...
      const tail = this.lastOutcome === 'paused'
        ? ', because it is backing off after repeated failures'
    ...
    if (this.lastOutcome === 'failed') {
      return `The queue loop finished a pass ${gap} ago and that pass failed.`;
    }
