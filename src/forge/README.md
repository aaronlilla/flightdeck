...
`forge stop --all` is the one command documented as having to work "when nothing else does." It does not kill outright. It parks every live run with a handoff packet first, so the work survives and `forge up` can pick each one back up later. The CLI's own help text is blunt about the scope: it will "park every run with a handoff and end all spend."
...
```
npm run verify   # typecheck, tests, agnostic path check, fault injection
```
